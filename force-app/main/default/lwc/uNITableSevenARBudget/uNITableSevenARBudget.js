...
import getTableData from '@salesforce/apex/uNI_TableSevenARBudgetController.getTableData';
...
export default class UNITableSevenARBudget extends LightningElement {
    @api recordId;
    @track loading = false;
    @track error = '';
    ...
    wiredAR({ data, error }) {
        if (data) {
            ...
            console.error('[Table7] Error fetching AR fields', error);
        }
    }
    ...
    loadData() {
        ...
        getTableData({ recordId: recIdToUse })
            .then((res) => {
                ...
                console.log('[Table7] outputsCount resolved:', outputsCount, 'controller:', res.numberOfOutputs);
                console.log('[Table7] expenseTypes sample:', JSON.stringify((res.expenseTypes || []).slice(0, 3)));
    ...
    buildCountryRows(expenseTypes, outputsCount) {
        ...
        const getBucket = (country) => {
            if (!country) return null; // skip unspecified
            const key = country;
            const label = this.countryNames[country] || country;
            if (!map.has(key)) {
                map.set(key, { key, label, budget: 0, actual: 0 });
            }
            return map.get(key);
        };
        ...
        expenseTypes.forEach((rec) => {
            for (let i = 1; i <= outputsCount; i++) {
                const countryField = `uNI_Output${i}Country__c`;
                const countryVal = rec[countryField];
                const bucket = getBucket(countryVal);
                if (bucket) {
                    bucket.budget += this.toNumberSafe(rec[`uNI_ProjectedExpenseOutput${i}__c`]) || 0;
                    bucket.actual += this.toNumberSafe(rec[`uNI_Output${i}CountryActual__c`]) || 0;
                }
            }
            const crossCountry = rec.uNI_CrosscuttingCountry__c;
            const crossBucket = getBucket(crossCountry);
            if (crossBucket) {
                crossBucket.budget += this.toNumberSafe(rec.uNI_ProjectedExpenseCrossCutting__c) || 0;
                crossBucket.actual += this.toNumberSafe(rec.uNI_CrosscuttingCountryActual__c) || 0;
            }
        });
        ...
        map.forEach((bucket) => {
            ...
            const pct = bucket.budget === 0 ? '' : `${((bucket.actual / bucket.budget) * 100).toFixed(2)}%`;
            rows.push({
                key: bucket.key,
                label: bucket.label,
                ...
                variance,
                pct,
                yearBudget: bucket.budget,
                yearActual: bucket.actual,
